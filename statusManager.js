...
async function initStatusManager() {
  ...
  if (fileSaved) {
    if (fileSaved.lastOnlineAt) state.lastOnlineAt = fileSaved.lastOnlineAt;
    if (fileSaved.lastOfflineAt) state.lastOfflineAt = fileSaved.lastOfflineAt;
    if (fileSaved.lastCrashAt) state.lastCrashAt = fileSaved.lastCrashAt;
    if (fileSaved.queue) state.queue = fileSaved.queue;
    if (fileSaved.lastOfflineAlertTime) state.lastOfflineAlertTime = fileSaved.lastOfflineAlertTime;
    if (fileSaved.offlineAlertCount) state.offlineAlertCount = fileSaved.offlineAlertCount;
    if (fileSaved.lastOfflineReason) state.lastOfflineReason = fileSaved.lastOfflineReason;
    if (fileSaved.lastOnlineTime) state.lastOnlineTime = fileSaved.lastOnlineTime;
    console.log("Restored state from persistence (file-only)");
  }
}
